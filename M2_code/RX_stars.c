...
struct robots {             // Structure for variables of each robot
    ...
    int y3;
    int y4;
} robot1, robot2, robot3, robot4;   // Robot 1,2,3 variables
	...
	while (1)
	{
		if (flag_data == 1)     // If we received from RF from robots
		{
            ...
            if (robot_num == 1) {               // If we received from robot 1
                ...
                robot4.x1 = (int)buffer_rec[1]*128 + (int)buffer_rec[2];
                robot4.x2 = (int)buffer_rec[3]*128 + (int)buffer_rec[4];
                robot4.x3 = (int)buffer_rec[5]*128 + (int)buffer_rec[6];
                robot4.x4 = (int)buffer_rec[7]*128 + (int)buffer_rec[8];
                robot4.y1 = (int)buffer_rec[9]*128 + (int)buffer_rec[10];
                robot4.y2 = (int)buffer_rec[11]*128 + (int)buffer_rec[12];
                robot4.y3 = (int)buffer_rec[13]*128 + (int)buffer_rec[14];
                robot4.y4 = (int)buffer_rec[15]*128 + (int)buffer_rec[16];
            }
    ...
    if (USB){
        ...
        if(data_to_read)
        {
            ...
            if (flag == 0) {                    // If we haven't received anything before
                ...
                else if (flag == 9){                 // If MATLAB needs the Position and Orientation data send them ASAP
                    m_usb_rx_flush();           // Flush the RX buffer
                    
                    m_usb_tx_int(robot4.x1);
                    m_usb_tx_string("\n");
                    m_usb_tx_int(robot4.x2);
                    m_usb_tx_string("\n");
                    m_usb_tx_int(robot4.x3);
                    m_usb_tx_string("\n");
                    m_usb_tx_int(robot4.x4);
                    m_usb_tx_string("\n");
                    m_usb_tx_int(robot4.y1);
                    m_usb_tx_string("\n");
                    m_usb_tx_int(robot4.y2);
                    m_usb_tx_string("\n");
                    m_usb_tx_int(robot4.y3);
                    m_usb_tx_string("\n");
                    m_usb_tx_int(robot4.y4);
                    m_usb_tx_string("\n");
